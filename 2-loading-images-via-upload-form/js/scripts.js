...
function initImageLoader() {
  uploadedFile.addEventListener("change", handleManualUploadedFiles, false);
  
  function handleManualUploadedFiles(response) {
    var _file = response.target.files[0];
    handleFile(_file);
  }
}
...
function handleFile(file) {
    ...
    if (file.type.match(imageType)) {
        ...
        reader.onloadend = function(event) {
          ...
          _tempImageStore.src = event.target.result; // Put the loaded image into our new image object, triggering the above onload function.
        }
